                         ...
                         char *buf, int buf_size)
{
    ...
    double sl = (double)sunload_x100 / 100.0;
    double tx = (double)txpower_x10  / 10.0;

    calculate_state(a, sl, tx, model, &state);
    ...
    snprintf(buf, buf_size,
        ...
        (int)state.battery_level, (int)model->battery_capacity,
        (int)state.generated_power,
        (int)state.temperature,
        (int)model->temp_min, (int)model->temp_max,
        state.mode
    );
}
